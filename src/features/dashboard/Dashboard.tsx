    ...
    <div className="max-w-7xl mx-auto flex flex-col space-y-10 animate-in fade-in duration-500 pb-12">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-muted-foreground mt-1.5">
            Real-time enterprise telemetry and operations.
          </p>
        </div>
        <Button
          variant="outline"
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-6">
        {kpis.map((kpi, idx) => (
          ...
          >
            <div
              className={`absolute top-0 right-0 w-32 h-32 -mr-12 -mt-12 rounded-full opacity-10 blur-3xl transition-all duration-500 group-hover:opacity-30 ${kpi.bg}`}
            ></div>
          ...
          <CardHeader className="border-b border-border bg-secondary/20">
            <CardTitle className="text-lg font-semibold flex items-center justify-between">
              <span className="flex items-center gap-2">
                <Bell className="h-4 w-4 text-primary" /> Live Operations Stream
              </span>
            ...
            {notifications.length === 0 ? (
              ...
              <div className="divide-y divide-border">
                {notifications.slice(0, 6).map((notif: any) => (
                  <div
                    key={notif.id}
                    className="p-4 hover:bg-secondary/30 transition-colors flex gap-4 items-start group"
                  >
                    <div
                      className={`mt-1 rounded-full p-1 border flex-shrink-0 transition-transform group-hover:scale-110
                      ${
                        notif.type === "alert"
          ...
          <div className="bg-destructive/10 border border-destructive/30 rounded-xl p-4 flex gap-3 shadow-sm relative overflow-hidden group">
            ...
            <div>
              ...
              <p className="text-xs text-destructive/80 mt-1">
                2 high-priority maintenance tickets pending approval.
              </p>
              <button className="text-xs font-bold text-destructive mt-2 hover:underline">
                Review Tickets &rarr;
              </button>
            </div>
          </div>
