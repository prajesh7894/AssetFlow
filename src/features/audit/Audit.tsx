...
export default function Audit() {
  ...
  // Mutations
  const { createRecord: createAuditLog, loading: mutatingAudit } =
    useFirestoreMutation("auditLogs");
      ...
      if (assetToUpdate) {
        if (action === "Update Location") {
          ...
          await updateAsset(assetToUpdate.id, {
            ...
            history: [
              ...(assetToUpdate.history || []),
              {
                ...
                action: `Audit resolution: Location updated to ${selectedLog.scannedLocation}`,
              },
            ],
          });
          ...
          await updateAsset(assetToUpdate.id, {
            ...
            history: [
              ...(assetToUpdate.history || []),
              { date: new Date().toISOString(), action: `Audit resolution: Marked as Missing` },
            ],
          });
        }
      }
  ...
  const getStatusIcon = (status: string) => {
    switch (status) {
      ...
      case "Missing":
        return <XCircle className="h-4 w-4 text-destructive mr-2" />;
      case "Resolved":
        return <ShieldCheck className="h-4 w-4 text-primary mr-2" />;
      default:
        return null;
    }
  };
  ...
  return (
    <div className="max-w-7xl flex flex-col h-full animate-in fade-in duration-500">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between mb-8 gap-4">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-1">
            Physical inventory reconciliation and compliance tracking.
          </p>
        </div>
              ...
              [...auditLogs]
                ...
                .map((log: any) => (
                  <TableRow key={log.id} className="group">
                    ...
                    <TableCell
                      className={
                        log.expectedLocation !== log.scannedLocation
      ...
      <Dialog
        ...
        title="Continuous Audit Scanner"
      >
        <ScannerSimulator
          assets={assets}
          ...
          <div className="space-y-6">
            <div className="bg-amber-500/10 border border-amber-500/50 p-4 rounded-xl text-amber-600">
              ...
              <p className="text-sm opacity-90">
                Asset <span className="font-mono font-bold">{selectedLog.assetTag}</span> was
                expected in <b>{selectedLog.expectedLocation}</b> but was physically scanned in{" "}
                <b>{selectedLog.scannedLocation}</b>.
              </p>
            </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                >
                  ...
                  <p className="text-xs text-muted-foreground">
                    Accept the scanned location as the new truth. This updates the asset registry
                    permanently.
                  </p>
                </div>
                ...
                >
                  ...
                  <p className="text-xs text-muted-foreground">
                    Flag this asset for investigation. It will be marked as 'Missing' in the
                    database.
                  </p>
                </div>
              </div>
