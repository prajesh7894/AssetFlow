...
export default function Reports() {
  ...
  const totalAssetValue = assets.reduce(
    (sum: number, asset: any) => sum + (Number(asset.cost) || 0),
    0
  );

  const openTickets = maintenance.filter(
    (t: any) => t.status !== "Resolved" && t.status !== "Rejected"
  ).length;
  const maintLoad = maintenance.length > 0 ? (openTickets / maintenance.length) * 100 : 0;

  const totalAudits = auditLogs.length;
  const verifiedAudits = auditLogs.filter(
    (a: any) => a.status === "Match" || a.status === "Resolved"
  ).length;
  ...
  maintPriorityData.sort(
    (a: any, b: any) => (priorityOrder[a.name] || 99) - (priorityOrder[b.name] || 99)
  );
      ...
      <div className="flex flex-col md:flex-row justify-between md:items-center gap-4">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-1">
            Cross-module insights and data exports.
          </p>
        </div>
          ...
          <CardContent className="p-6">
            <div className="flex items-center justify-between">
              <div>
                ...
                <h3 className="text-3xl font-bold tracking-tight text-amber-500">
                  {maintLoad.toFixed(1)}%
                </h3>
              </div>
            ...
            <p className="text-xs text-muted-foreground mt-4">
              {openTickets} active tickets pending resolution.
            </p>
          </CardContent>
        </Card>
      </div>
