...
export default function Organization() {
  ...
  const getDeptAssets = (deptId: string) => {
    ...
    return assets.filter(
      (a) => a.status === "Allocated" && deptEmployeeNames.includes(a.assignedTo)
    );
  };
    ...
    <div className="max-w-7xl flex flex-col h-full animate-in fade-in duration-500 pb-10">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between mb-8 gap-4">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-1">
            Manage personnel, departments, and structure.
          </p>
        </div>
                  ...
                  {employees.length === 0 ? (
                    ...
                    employees.map((emp) => {
                      ...
                      const heldAssets = assets.filter(
                        (a) => a.status === "Allocated" && a.assignedTo === emp.name
                      );
      ...
      >
        <EmployeeForm
          ...
          onSubmit={handleCreateEmp}
          isSubmitting={mutatingEmp}
        />
      </Dialog>
    </div>
  );
