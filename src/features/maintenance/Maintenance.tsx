  ...
  const handleUpdateStatus = async (ticket: any, newStatus: string) => {
    try {
      await updateRecord(ticket.id, {
        ...
        history: [
          ...(ticket.history || []),
          { date: new Date().toISOString(), action: `Moved to ${newStatus}` },
        ],
      });
          ...
          {loading ? (
            ...
            KANBAN_COLUMNS.map((column) => {
              ...
              return (
                <div
                  key={column}
                  className="w-80 flex flex-col bg-secondary/10 border border-border/50 rounded-xl overflow-hidden h-full"
                >
                  {/* Column Header */}
                    ...
                    {colTickets.map((ticket) => (
                      ...
                      >
                        <div className="flex justify-between items-start mb-2">
                          ...
                          <span
                            className={`text-[10px] px-1.5 py-0.5 rounded border ${getPriorityColor(ticket.priority)}`}
                          >
                            {ticket.priority}
                          </span>
                        </div>
          ...
          <div className="flex flex-col md:flex-row gap-6">
            ...
            <div className="flex-1 space-y-4">
              <div className="flex items-center gap-2">
                <span className="bg-secondary text-muted-foreground px-2 py-1 rounded-md text-xs font-mono">
                  {selectedTicket.assetId}
                </span>
                <span
                  className={`text-xs px-2 py-1 rounded border ${getPriorityColor(selectedTicket.priority)}`}
                >
                  {selectedTicket.priority} Priority
                </span>
                <span className="bg-primary/20 text-primary border border-primary/50 px-2 py-1 rounded text-xs">
              ...
              {selectedTicket.image && (
                <div>
                  ...
                  <img
                    ...
                    alt="Evidence"
                    className="rounded-md border border-border max-h-48 object-contain bg-secondary/20"
                  />
                </div>
              )}
